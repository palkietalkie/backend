...
- Inference plane: pluggable via `INFERENCE_PROVIDER` env var on the Fly backend.
  - `openai` (current default) — calls OpenAI Realtime via `app/services/openai_realtime.py:mint_openai_session`. Returns an ephemeral `client_secret.value` (string like `ek_…`) + `wss://api.openai.com/v1/realtime?model=gpt-realtime-2`. JSON event protocol. Requires `OPENAI_API_KEY`. Server-side VAD with `interrupt_response: true` configured at session-mint time.
...
- `app/post_session_nlp/` — the jobs that run once a conversation ends, turning its transcript into derived data: `kg_extraction`, `mistake_detection`, `phrase_extraction`, `transcript_analysis`, `transcript_embedding`. Each split into a pure `extract(...)` + async `persist(...)` pair so the LLM-touching half stays unit-testable. (Formerly the ambiguous `app/pipelines/`, which also mixed in unrelated scheduled jobs.)
- `app/daily_content/` — scheduled (cron) generator for the "what to talk about today" news + quizzes. Not per-session.
- `app/audio_retention/` — scheduled prune of expired session-audio rows.
...
- `app/asc/` + `scripts/asc/` — App Store Connect metadata SSoT + push scripts (listing copy, App Privacy declaration, screenshots). ASC TEXT FIELDS (name / subtitle / description / promotional text / keywords) must NOT contain em or en dashes — Apple's listing reads cleaner with plain punctuation, so use commas / periods / parens. Dashes are fine everywhere else (HTML page titles, docs, code comments); this ban is ASC-copy only.
