...
def reserve_review_screenshot(client: httpx.Client, sub_id: str, png: Path) -> dict[str, Any]:
    ...
    Returns the full `data` block (asset id + attributes); caller passes `attributes.uploadOperations` to `upload_asset_bytes` and the asset id to `commit_review_screenshot`.
    """
