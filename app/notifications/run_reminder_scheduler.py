...
async def run_reminder_scheduler() -> None:
    """Wake at the top of every hour and run each per-tick notification pass (daily reminder, streak warning, weekly recap). Every pass's candidate query matches users by their OWN local time, so hourly granularity is enough (the targets are whole hours), and the notification_log dedup guards against a restart re-running a tick."""
    while True:
        ...
        try:
            ...
            if daily or streak_warning or weekly_recap:
                logger.info(
                    "reminders: pushed %d daily, %d streak-warning, %d weekly-recap",
                    daily,
                    streak_warning,
                    weekly_recap,
                )
