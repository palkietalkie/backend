...
from app.services.cefr_vocab._data import CSV_PATH


def test_vocab_csv_lives_under_app_tree() -> None:
    app_dir = Path(__file__).resolve().parents[2]
    assert app_dir.name == "app"
    assert CSV_PATH.is_relative_to(app_dir), (
        f"data.csv at {CSV_PATH} must live under {app_dir}/ so the Dockerfile's `COPY app ./app` "
...
def test_vocab_csv_exists() -> None:
    assert CSV_PATH.is_file(), (
        f"data.csv missing at {CSV_PATH} — regenerate via scripts/regenerate_cefr_vocab.py"
    )
